/*
   ...
   Special thanks to:
      ...
      Günter Beyes for supplying an IE7 fix
...
METHOD XbFTP:init( cAddress, cUserId, cPassword, cProxy, nPort )
   local i
...
METHOD XbFTP:iClose()
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"InternetCloseHandle")
   endif
...
METHOD XbFTP:getCurrentDirectory()
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"FtpGetCurrentDirectoryA")
   endif
...
METHOD XbFTP:setCurrentDirectory(cDirectry)
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"FtpSetCurrentDirectoryA")
   endif
...
METHOD XbFTP:createDirectory(cDirectry)
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"FtpCreateDirectoryA")
   endif
...
METHOD XbFTP:deleteDirectory(cDirectry)
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"FtpRemoveDirectoryA")
   endif
...
METHOD XbFTP:deleteFile(cFile)
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"FtpDeleteFileA")
   endif
...
METHOD XbFTP:renameFile(cFile, cNewName)
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"FtpRenameFileA")
   endif
...
METHOD XbFTP:getFile(cRemoteFile, cLocalFile, lOverWrite, fAttr, nTransferMode)
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"FtpGetFileA")
   endif
...
METHOD XbFTP:putFile(cLocalFile, cRemoteFile, nTransferMode)
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"FtpPutFileA")
   endif
...
METHOD XbFTP:findNext()
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"InternetFindNextFileA")
   endif
...
METHOD XbFTP:sendCommand(cCommand)
   ...
   local lOk := FALSE
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"FtpCommandA")
   endif
   if !Empty(cCommand)
...
METHOD XbFTP:sendFile( cLocalFile, cRemoteFile, bCallBack, nReTries )
   ...
   if nIn == 0
      ...
      if nOut == 0
         ...
         if aCB.TotalSent == nEnd
            aCB.Status := FTRANS_COMPLETE
         endif
...
METHOD XbFtp:ftpOpenFile( cFile, nMode )
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"FtpOpenFileA")
   endif
...
METHOD XbFtp:inetWriteFile( nOut, cBuff, nRead, nRemRead )
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"InternetWriteFile")
   endif
...
METHOD XbFtp:inetReadFile( nOut, cBuff, nRead, nRemRead )
   ...
   if cTpl == NIL
      cTpl := DllprepareCall("wininet.dll",DLL_STDCALL,"InternetReadFile")
   endif
   lOk := DllExecuteCall( cTpl, nOut, @cBuff, nRead, @nRemRead ) > 0
   return (lOk)
